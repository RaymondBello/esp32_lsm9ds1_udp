// From Tao_spartan from post on esp32.com https://esp32.com/viewtopic.php?t=7882#p33124
...
const char * udpAddress = "192.168.0.153";//mac
...
const unsigned int PRINT_RATE = 50;
...
//Serial.print("A: ");
//Serial.print();
float x_accel = imu.calcAccel(imu.ax);
Serial.print(x_accel);
float y_accel = imu.calcAccel(imu.ay);
Serial.print(y_accel);
float z_accel = imu.calcAccel(imu.az);
Serial.print(z_accel);
Serial.print("\n");

float x_gyro = imu.calcGyro(imu.gx);
Serial.print(x_gyro);
float y_gyro = imu.calcGyro(imu.gy);
Serial.print(y_gyro);
float z_gyro = imu.calcGyro(imu.gz);
Serial.print(z_gyro);
Serial.print("\n");
Serial.print("mag x");
float x_mag = imu.calcMag(imu.mx);
Serial.print(x_mag);
Serial.print("mag y");
float y_mag = imu.calcMag(imu.my);
Serial.print(y_mag);
Serial.print("mag z");
float z_mag = imu.calcMag(imu.mz);
Serial.print(z_mag);
Serial.print("\n");
Serial.print("new");
...
udp.beginPacket(udpAddress, udpPort);
    ...
    sprintf(msg, "%f,%f,%f,%f,%f,%f,%f,%f,%f",x_accel,y_accel,z_accel,x_gyro,y_gyro,z_gyro,x_mag,y_mag,z_mag);
